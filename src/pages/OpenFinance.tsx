...
import { MultipleAccountsManager } from "@/components/open-finance/MultipleAccountsManager";
...
const OpenFinance = () => {
  ...
  const handleRefreshAccountBalance = async (integration: any) => {
    if (!integration.item_id) return;
    
    setRefreshingBalance(integration.id);
    try {
      await refreshBalance(integration.item_id, true);
      toast({
        title: "Saldo atualizado",
        description: `Saldo da conta ${integration.nome_banco} foi atualizado com sucesso.`,
      });
    } catch (error) {
  ...
  // Se já existem integrações, mostrar interface de múltiplas contas
    ...
    return (
      <AppLayout>
        <div className="max-w-6xl mx-auto py-8 space-y-6">
          <div className="flex justify-between items-center">
            <div>
              ...
              <p className="text-muted-foreground">
                Gerencie suas contas bancárias conectadas e adicione novas conexões
              </p>
            </div>
            
            <Button onClick={handlePluggyConnect} disabled={isConnecting || !isScriptLoaded}>
              {isConnecting ? (
                <>
          ...
          <MultipleAccountsManager
            integrations={activeIntegrations}
            onAddNewAccount={handlePluggyConnect}
            onRefreshAccount={handleRefreshAccountBalance}
            refreshingBalance={refreshingBalance}
            formatDate={formatDate}
          />

          {/* Seção de sincronização rápida para todas as contas */}
          <Card>
            <CardHeader>
              <CardTitle>Sincronização de Dados</CardTitle>
              <CardDescription>
                Sincronize transações de todas as contas conectadas
              </CardDescription>
            </CardHeader>
            <CardContent>
                    ...
                    onClick={async () => {
                      for (const integration of activeIntegrations) {
                        if (syncing !== integration.id) {
                          await handleSyncData(integration.id);
                        }
                      }
                    }}
                    disabled={!!syncing}
  ...
  // Se não estiver carregando conexão, mostrar interface de conexão
  return (
    <AppLayout>
      <div className="max-w-4xl mx-auto py-8">
            ...
            <div className="text-center space-y-6">
              ...
              <div className="grid md:grid-cols-3 gap-6">
                <div className="text-center">
                  ...
                  <h4 className="font-medium text-gray-900 mb-1">Segurança Total</h4>
                  <p className="text-sm text-gray-600">Certificação OpenFinance</p>
                </div>
                <div className="text-center">
                  <div className="w-12 h-12 bg-blue-100 rounded-lg flex items-center justify-center mx-auto mb-3">
                    <CreditCard className="w-6 h-6 text-blue-600" />
                  </div>
                  <h4 className="font-medium text-gray-900 mb-1">Dados Reais</h4>
                ...
                <div className="text-center">
                  ...
                  <h4 className="font-medium text-gray-900 mb-1">IA Avançada</h4>
                  <p className="text-sm text-gray-600">Insights financeiros</p>
                </div>
              </div>
